...
import logging
from argot.graphs import ArgotGraph
from argot.data_domains import DataDomains
from util.thread_data import ThreadData
...
class DomainsThread(Process):
    ...
    def get_neighbors(self, map: Dict[int, Set[str]], pid: str, blast_prot: Set[str]):
        try:
            ...
            if not dd.getExit():
                uidscore = dd.getResults().getUidOver(self.nwth)
                if uidscore:
                    at = ArgotGraph(uidscore, pid, self.database, blast_prot, self.ident)
                    if pid not in self.output:
                        self.output[pid] = ThreadData(at.getProtClusters(), at.getCluster())
            else:
