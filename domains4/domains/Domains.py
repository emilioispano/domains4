...
from utils import ThreadData
from utils import Utilities
from parsers.OptionsParser import OptionsParser
    ...
    def write_results(self, results: Dict[str, ThreadData], collgoa: Collection) -> None:
        for template in results.keys():
            ...
            for uid in results[template].get_neighbor_map().keys():
                if results[template].get_prot_clusters() is not None and uid in results[template].get_prot_clusters():
                    for go_id in results[template].get_prot_clusters()[uid].get_gos():
                        self.setting.get_out().write(f"{go_id}\t{results[template].get_neighbor_map()[uid]}\t{uid}\t0\n")
                else:
                    for doc in collgoa.find({"uid": uid}):
                        go_id = doc["goid"]

                        if uid not in results[template].get_prot_clusters() or go_id not in results[template].get_prot_clusters()[uid].get_gos():
